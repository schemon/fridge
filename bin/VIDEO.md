...
(Direct scripts still exist under `bin/`, but `./bin/fridge ...` is the preferred interface.)

## Deterministic frames (for tracking)

Extract frames at exact timestamps (e.g. every 100ms) and name them by time:
```bash
./bin/fridge frames extract ./captures/video-YYYY.../clip.mp4 --step-ms 100 --duration-s 5
